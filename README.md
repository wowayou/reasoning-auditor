...
仓库还包含一个纯静态 GitHub Pages 版本。它运行浏览器内置的确定性 Mock，不需要后端、不需要 API Key，也不会把观点发送到任何供应商。推送到 `main` 后，`.github/workflows/pages.yml` 会构建并发布 `site/`。

静态版适合产品演示、UI 验收和分享；它不会调用真实模型。原因是 GitHub Pages 是公开静态站点，把供应商 Key 放进浏览器会暴露密钥，而且供应商常常限制 CORS/客户端调用。真实模型请使用 Docker/FastAPI 版本；用户自己的 Key 也只能在受信任的后端代理上使用。
